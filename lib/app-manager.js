			...
			if (App.isManager) {
				if (msg.type == 'mgrCommand') {
					...
					case 'watchlog':
						...
						var startByte = 0;
						var app = self.applications[msg.appID];

						fs.stat(app.appLogPath, function(err, stats){
							if (err)
								throw err;
							...
							fs.watch(app.appLogPath, function(event, filename) {
								fs.stat(app.appLogPath, function(err, stats){
									if (err)
										throw err;

									/* Open log file */
									var fileStream = fs.createReadStream(app.appLogPath, {
										start: startByte,
										end: stats.size
									})
					...
					case 'unwatchlog':
						...
						/* Unwatch log file */
						fs.unwatchFile(self.applications[msg.appID].appLogPath);

						/* Tell worker what we do not watch log anymore */
						appProcess.send({
							type: 'callback',
							callback: msg.callback,
							args: [ null ]
